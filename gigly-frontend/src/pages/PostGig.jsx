...
export default function PostGig() {
  ...
  const payPerWorker = (parseFloat(form.payPerHour)||0) * (parseInt(form.durationHours)||0);
  const totalPay = payPerWorker * (parseInt(form.slotsRequired)||1);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data: jobRes } = await jobsAPI.create(payload);
      const jobId = jobRes.data._id;
      
      // 2. Create Razorpay Order
      const { paymentsAPI } = await import("../services/api");
      const { data: orderRes } = await paymentsAPI.createOrder(jobId);
      ...
      const options = {
        ...
        name: "Rozgaaar",
        description: `Payment for Gig: ${form.title}`,
        ...
        handler: async function (response) {
          try {
            ...
            toast.success("Payment verified! Gig is now LIVE 🎉");
            navigate(`/jobs/${jobId}`);
          } catch (err) {
      ...
      rzp.on('payment.failed', function (response){
        toast.error("Payment failed. Job saved as draft.");
      });
      rzp.open();
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              ...
              <div className="grid-2" style={{ gap:14 }}>
                ...
                <div className="input-group">
                  ...
                  <div className="input" style={{ background:"var(--bg-base)", color:"var(--text-secondary)", display: "flex", alignItems: "center" }}>
        ...
        <div className="post-gig-preview fade-in fade-in-3" style={{ position:"sticky", top:96 }}>
          ...
          <div className="card" style={{ border:"1px solid var(--border-active)" }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <div>
                <div style={{ fontSize:11, color:"var(--text-muted)", marginBottom:2 }}>Total Escrow ({form.slotsRequired} worker{form.slotsRequired > 1 ? "s" : ""})</div>
                ...
                <div style={{ fontSize:11, color:"var(--text-muted)", marginTop: 6, marginBottom:2 }}>You Pay (+7% Fee)</div>
                <div style={{ fontFamily:"var(--font-display)", fontWeight:800, fontSize:22, color:"var(--accent)" }}>
                  {totalPay > 0 ? `₹${(totalPay * 1.07).toLocaleString("en-IN")}` : "₹—"}
                </div>
              </div>
