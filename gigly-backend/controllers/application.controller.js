...
exports.updateApplicationStatus = asyncHandler(async (req, res, next) => {
  ...
  if (status === "accepted") {
    ...
    await User.findByIdAndUpdate(application.worker._id, {
      $inc: { totalJobsCompleted: 1, totalEarnings: application.totalPaid, walletBalance: application.totalPaid }
    });
    await User.findByIdAndUpdate(application.business, {
      $inc: { totalSpent: application.totalPaid, walletBalance: -application.totalPaid }
    });
