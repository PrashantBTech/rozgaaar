...
// ── GET /api/v1/admin/growth – Weekly registrations (last 12 weeks) ──────────
router.get(
  "/growth",
  ...adminGuard,
  asyncHandler(async (req, res) => {
    ...
    for (let i = weeks - 1; i >= 0; i--) {
      ...
      const [workers, businesses, jobs] = await Promise.all([
        User.countDocuments({ role: "worker",   createdAt: { $gte: weekStart, $lte: weekEnd } }),
        User.countDocuments({ role: "business", createdAt: { $gte: weekStart, $lte: weekEnd } }),
        Job.countDocuments({                    createdAt: { $gte: weekStart, $lte: weekEnd } }),
      ]);

      workerData.push(workers);
      businessData.push(businesses);
      jobData.push(jobs);
    }

    res.json({ success: true, data: { labels, workerData, businessData, jobData } });
  })
);
